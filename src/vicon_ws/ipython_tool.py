...
def build_user_namespace() -> dict[str, object]:
    ...
    from vicon_ws import ape_tool, rpe_tool, traj_tool
    from vicon_ws.benchmark import metrics_res, plot_summary, res
    from vicon_ws.core import calibration, evaluation, io_utils, math_utils, pipeline_modular, time_alignment
...
def launch_ipython(ipython_args: list[str], user_ns: dict[str, object], banner: str) -> int:
    try:
        ...
        raise ImportError(
            "IPython is required for vicon_ws_ipython. Install with: pip install ipython"
        ) from exc
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Launch IPython with pre-loaded vicon_ws modules."
    )
    p.add_argument(
        "--list",
        action="store_true",
