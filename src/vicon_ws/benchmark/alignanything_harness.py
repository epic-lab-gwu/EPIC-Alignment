...
def _parse_vicon_run_dir(stdout_text: str, stderr_text: str) -> Path | None:
...
def _run_vicon_case(
    ...
    *,
    ...
    run_dir = _parse_vicon_run_dir(proc.stdout, proc.stderr)
    if run_dir is None:
        return result
    ...
    for row in rows:
        lines.append(
            "| {case} | {status} / {estatus} | {vr} / {er} | {va} / {ea} | {vi} / {ei} | {vo} / {eo} | {vm} / {em} |".format(
                case=row.get("case", ""),
                status=row.get("vicon_status", ""),
                estatus=row.get("evo_status", ""),
                vr=_fmt(row.get("vicon_ate_rmse_raw_m"), 3),
                va=_fmt(row.get("vicon_ate_rmse_step3_m"), 3),
                er=_fmt(row.get("evo_ape_raw_rmse_m"), 3),
                ea=_fmt(row.get("evo_ape_se3_rmse_m"), 3),
                vi=_fmt(row.get("vicon_improve_pct"), 2),
                ei=_fmt(row.get("evo_improve_pct"), 2),
                vo=_fmt(row.get("vicon_offset_est_s"), 3),
                eo=_fmt(row.get("evo_offset_s"), 3),
                vm=_fmt(row.get("vicon_matches_equivalent"), 0),
    ...
    for row in rows:
        lines.append(
            "| {case} | {peak} | {psr} | {omega} |".format(
                case=row.get("case", ""),
                peak=_fmt(row.get("vicon_xcorr_peak"), 3),
                psr=_fmt(row.get("vicon_xcorr_psr"), 2),
                omega=_fmt(row.get("vicon_omega_improve_pct"), 2),
            )
        )
    lines.extend(
        [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Independent benchmark harness for AlignAnything: vicon_ws vs evo."
    )
    parser.add_argument(
        "--alignanything-root",
    ...
    parser.add_argument(
        ...
        help="Where harness outputs are written.",
    )
    parser.add_argument(
        "--repo-root",
        default=".",
        help="Repository root for running vicon_ws CLI.",
    )
    parser.add_argument(
        "--python-bin",
        default="/home/yifu/miniconda3/envs/vicon_ws310/bin/python",
        help="Python executable used to run vicon_ws (should be py3.10+).",
    )
    parser.add_argument(
        "--vicon-src",
        default="src",
        help="vicon_ws source dir to append into PYTHONPATH.",
    )
    parser.add_argument(
        "--evo-repo",
    ...
    parser.add_argument("--dt-resample", type=float, default=0.001, help="vicon_ws dt_resample.")
    parser.add_argument(
        "--quat-interp",
        choices=["linear", "slerp"],
        default="linear",
        help="vicon_ws quaternion interpolation mode.",
    )
    parser.add_argument(
        "--t-max-diff",
        type=float,
...
def run(args: argparse.Namespace) -> int:
    ...
    vicon_src = (repo_root / args.vicon_src).resolve()
    ...
    for index, case in enumerate(cases, start=1):
        ...
        case_payload: dict[str, object] = {
            ...
            "status": "failed",
            "offset_policy": "vicon_internal + evo_independent_sweep",
        }
        try:
            ...
            summary_rows.append(
                {
                    ...
                    "vicon_status": "failed",
                    "evo_status": "failed",
                    "vicon_offset_est_s": float("nan"),
                    "evo_offset_s": float("nan"),
                    "vicon_ate_rmse_raw_m": float("nan"),
                    "vicon_ate_rmse_step3_m": float("nan"),
                    "evo_ape_raw_rmse_m": float("nan"),
                    "evo_ape_se3_rmse_m": float("nan"),
                    "vicon_improve_pct": float("nan"),
                    "evo_improve_pct": float("nan"),
                    "vicon_xcorr_peak": float("nan"),
                    "vicon_xcorr_psr": float("nan"),
                    "vicon_omega_improve_pct": float("nan"),
                    "vicon_matches_equivalent": float("nan"),
        ...
        vicon_stdout_log = logs_dir / "vicon" / f"{case.case_id}.stdout.log"
        vicon_stderr_log = logs_dir / "vicon" / f"{case.case_id}.stderr.log"
        vicon_result = _run_vicon_case(
            case,
            gt_tum,
            est_tum,
            repo_root=repo_root,
            ...
            stdout_log_path=vicon_stdout_log,
            stderr_log_path=vicon_stderr_log,
        )
        ...
        both_ok = (vicon_result.get("status") == "ok") and (evo_result.get("status") == "ok")
        row = {
            "case": case.case_id,
            "dataset": case.dataset,
            "method": case.method,
            ...
            "vicon_status": vicon_result.get("status", "failed"),
            "evo_status": evo_result.get("status", "failed"),
            "vicon_offset_est_s": _as_float(vicon_result.get("offset_est_s")),
            "evo_offset_s": _as_float(evo_result.get("offset_s")),
            "vicon_ate_rmse_raw_m": _as_float(vicon_result.get("ate_rmse_raw_m")),
            "vicon_ate_rmse_step3_m": _as_float(vicon_result.get("ate_rmse_step3_m")),
            "evo_ape_raw_rmse_m": _as_float(evo_result.get("ape_raw_rmse_m")),
            "evo_ape_se3_rmse_m": _as_float(evo_result.get("ape_se3_rmse_m")),
            "vicon_improve_pct": _as_float(vicon_result.get("improve_raw_to_step3_pct")),
            "evo_improve_pct": _as_float(evo_result.get("improve_pct")),
            "vicon_xcorr_peak": _as_float(vicon_result.get("xcorr_peak")),
            "vicon_xcorr_psr": _as_float(vicon_result.get("xcorr_psr")),
            "vicon_omega_improve_pct": _as_float(vicon_result.get("omega_improve_pct")),
            "vicon_matches_equivalent": _as_float(vicon_result.get("evo_matches_equivalent")),
            "evo_matches": _as_float(evo_result.get("matches")),
            "vicon_run_dir": str(vicon_result.get("run_dir", "")),
            "vicon_stdout_log": str(vicon_result.get("stdout_log", "")),
            "vicon_stderr_log": str(vicon_result.get("stderr_log", "")),
            "evo_sweep_evals": int(evo_result.get("sweep_evals", 0)),
            "error": "",
        }
        if row["status"] != "ok":
            row["error"] = str(vicon_result.get("stderr_tail", "") or evo_result.get("error", ""))
