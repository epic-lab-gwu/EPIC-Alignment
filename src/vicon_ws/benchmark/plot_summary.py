...
os.environ.setdefault("MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "matplotlib-vicon_ws"))
...
NUMERIC_KEYS = {
    "vicon_offset_est_s",
    "evo_offset_s",
    "vicon_ate_rmse_raw_m",
    "vicon_ate_rmse_step3_m",
    "evo_ape_raw_rmse_m",
    "evo_ape_se3_rmse_m",
    "vicon_improve_pct",
    "evo_improve_pct",
    "vicon_xcorr_peak",
    "vicon_xcorr_psr",
    "vicon_omega_improve_pct",
    "vicon_matches_equivalent",
    "evo_matches",
    "evo_sweep_evals",
}
...
def _plot_status_counts(rows: list[dict[str, object]], out_dir: Path) -> Path:
    ...
    vicon_fail = sum(1 for row in rows if row.get("vicon_status") != "ok")
    evo_fail = sum(1 for row in rows if row.get("evo_status") != "ok")

    labels = ["both_ok", "both_fail", "vicon_fail", "evo_fail"]
    vals = [both_ok, both_fail, vicon_fail, evo_fail]
...
def _plot_offset_hist(rows: list[dict[str, object]], out_dir: Path) -> Path | None:
    vicon = _valid([float(row["vicon_offset_est_s"]) for row in rows])
    evo = _valid([float(row["evo_offset_s"]) for row in rows])
    if vicon.size == 0 and evo.size == 0:
        return None

    plt.figure(figsize=(8.5, 4.8))
    bins = 40
    if vicon.size > 0:
        plt.hist(vicon, bins=bins, alpha=0.55, label="vicon_offset_est_s", color="#00897b")
    if evo.size > 0:
...
def _plot_dataset_box(rows: list[dict[str, object]], out_dir: Path) -> Path | None:
    ...
    for row in rows:
        ...
        data_by_dataset[dataset][0].append(float(row["vicon_ate_rmse_step3_m"]))
    ...
    plt.title("Aligned RMSE by Dataset (vicon vs evo)")
    plt.legend([bp1["boxes"][0], bp2["boxes"][0]], ["vicon", "evo"], loc="upper right")
...
def _plot_top_cases(rows: list[dict[str, object]], out_dir: Path, top_k: int = 20) -> Path | None:
    ranked = []
    for row in rows:
        v = float(row["vicon_ate_rmse_step3_m"])
    ...
    plt.barh(y - h / 2.0, v_vals, height=h, color="#00897b", alpha=0.9, label="vicon")
...
def run(args: argparse.Namespace) -> int:
    ...
    v_aligned = np.asarray([float(row["vicon_ate_rmse_step3_m"]) for row in rows], dtype=float)
    e_aligned = np.asarray([float(row["evo_ape_se3_rmse_m"]) for row in rows], dtype=float)
    v_improve = np.asarray([float(row["vicon_improve_pct"]) for row in rows], dtype=float)
    e_improve = np.asarray([float(row["evo_improve_pct"]) for row in rows], dtype=float)
    v_matches = np.asarray([float(row["vicon_matches_equivalent"]) for row in rows], dtype=float)
    e_matches = np.asarray([float(row["evo_matches"]) for row in rows], dtype=float)

    maybe = _plot_log_scatter(
        v_aligned,
        e_aligned,
        "vicon aligned RMSE (m)",
        "evo aligned RMSE (m)",
        "Aligned RMSE: vicon vs evo",
    ...
    maybe = _plot_linear_scatter(
        ...
        "vicon improve (%)",
        "evo improve (%)",
        "Improve Percentage: vicon vs evo",
