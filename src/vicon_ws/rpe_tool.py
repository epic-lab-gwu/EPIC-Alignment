...
from vicon_ws.core.evaluation import RELATION_UNITS, compute_rpe_evo_style, normalize_pose_relation
from vicon_ws.core.io_utils import save_metrics, to_builtin, write_result_bundle
from vicon_ws.metric_cli_common import (
    MetricInputs,
    align_for_eval,
    apply_time_window,
    cum_distance,
...
def run(args: argparse.Namespace) -> int:
    ...
    if bool(getattr(args, "plot", False)) or str(getattr(args, "save_plot", "")).strip():
        ...
        x_vals, x_label = _pick_x_axis(rpe_block["_x_axis"], str(getattr(args, "plot_x_dimension", "seconds")), errs.size)
        ...
        _plot_map(
            ...
            pos_est=est_eval_pos,
            errors=errs,
            delta_ids=np.asarray(rpe_block["_x_axis"]["delta_ids"], dtype=int),
