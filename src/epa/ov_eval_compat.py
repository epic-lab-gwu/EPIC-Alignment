        ...
        for gt in gt_files:
            ...
            for run_file in run_files:
                try:
                    ...
                    source_details_ds.append(f"{run_file.name}:failed")
                    source_details_total.append(f"{algo_dir.name}/{ds}/{run_file.name}:failed")
                    continue
                ate_ori_rmse.append(float(ev["ate3_ori"]["rmse"]))
                ate_pos_rmse.append(float(ev["ate3_pos"]["rmse"]))
            ...
            if len(ate_ori_rmse) == 0:
                ...
                if source_details_ds:
                    print(f"\tfailed_runs: {_format_source_details(source_details_ds)}")
                continue
