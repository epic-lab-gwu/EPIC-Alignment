...
from epa.config_cli import get_global_config_path, resolve_scoped_config
...
def _generate_template(tool_name: str) -> dict:
    ...
    builders = {
        ...
        "epa_ape": "epa.ape_tool:_build_parser",
        "epa_rpe": "epa.rpe_tool:_build_parser",
    }
    ...
    if tool_name in {"epa_ape", "epa_rpe"}:
        cfg["subcommand"] = "tum"
    return cfg
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Global config manager for epa tools.")
    p.add_argument(
        "--config-path",
        default="",
        help="Custom settings path. Default: ~/.config/epa/settings.json",
    )
