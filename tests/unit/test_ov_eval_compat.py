...
def test_evaluate_pair_epa_step3_no_fallback_supports_low_rate_gt(
    tmp_path: Path,
) -> None:
    t_gt = np.arange(0.0, 30.001, 1.0)
    t_est = np.arange(0.025, 30.0, 0.05)
    ...
    assert result["ate3_pos"]["rmse"] < .01
...
def test_evaluate_pair_ov_style_no_fallback_supports_low_rate_gt(
    tmp_path: Path,
) -> None:
    t_gt = np.arange(0.0, 30.001, 1.0)
    t_est = np.arange(0.025, 30.0, 0.05)
