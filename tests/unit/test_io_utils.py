...
def test_write_run_reports_keeps_gallery_but_hides_alignment_debug_by_default(
    tmp_path: Path,
) -> None:
    out_dir = tmp_path / "run"
    ...
    assert report.index("step3_alignment_map.png") < report.index(
        "ape_translation_part_step3_series.png"
    )
    ...
    assert "debug_alignment_pipeline_3d.png" not in report


def test_write_run_reports_includes_alignment_debug_in_gallery_when_debug_enabled(
    tmp_path: Path,
) -> None:
    out_dir = tmp_path / "run"
    plots_dir = out_dir / "plots"
    ...
    assert "## User-Facing Figures" not in report
    assert "## Debug Figures" not in report
    assert report.index("step3_alignment_map.png") < report.index("debug_alignment_pipeline_3d.png")
    assert "step1_time_alignment.png" in report
...
def test_write_run_reports_highlights_time_rpe_and_pose_state_csv(tmp_path: Path) -> None:
    ...
    assert report.index("ape_translation_part_hist_p95.png") < report.index(
        "ape_translation_part_hist.png"
    )
    assert report.index("ape_translation_part_series_p95.png") < report.index(
        "ape_translation_part_hist.png"
    )
...
def test_make_output_dir_retries_existing_timestamp_names(
...
def test_make_output_dir_uses_safe_run_label(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    class FixedDateTime:
        @classmethod
