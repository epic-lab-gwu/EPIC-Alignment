...
def test_public_align_mode_collapses_compatibility_aliases() -> None:
    ...
    assert public_align_mode("epa_se3r") == "se3"
    assert public_align_mode("rotation_first_se3") == "se3"
    assert public_align_mode("se3-original") == "se3-original"
    assert public_align_mode("se3-orginal") == "se3-original"
    assert public_align_mode("umeyama_se3") == "se3-original"
...
def test_metric_eval_align_mode_preserves_advanced_modes_by_default() -> None:
    ...
    assert resolve_metric_eval_align_mode("epa_se3r") == "se3"
    assert resolve_metric_eval_align_mode("rotation_first_se3") == "se3"
    assert resolve_metric_eval_align_mode("se3-original") == "se3-original"
    assert resolve_metric_eval_align_mode("se3-orginal") == "se3-original"
